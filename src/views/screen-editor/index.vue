...
export default defineComponent({
  ...
  components: {
    ...
    FilterManagerDrawer: loadAsyncComponent(
      () => import("./filter-manager/filter-manager-drawer.vue")
    ),
  },
  props: {
    projectId: {
    ...
    onMounted(async () => {
      try {
        ...
        if (tplId > 0) {
          ...
          const screenId = +props.projectId; // 转number类型
          await Promise.all([
